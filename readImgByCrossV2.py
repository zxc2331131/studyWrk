...
def detect_dir_image(pr_result,do_count,file,do_typ=''):
    if (do_detect_flag == 1):
        if (do_typ == 'gaus'):
            model = keras.models.load_model('D:/KC_AOI_Project/wrk/study_wrk/modelLogs/G_Crosslogs_11_14/the-last-model.h5')
            # 預測的資料夾
            dir_path = 'D:/KC_AOI_Project/wrk/study_wrk/predict_pic/G_KC_Pic_Cross/'
        else:
            model = keras.models.load_model('D:/KC_AOI_Project/wrk/study_wrk/modelLogs/Crosslogs_11_14/the-last-model.h5')
            # 預測的資料夾
            dir_path = 'D:/KC_AOI_Project/wrk/study_wrk/predict_pic/KC_Pic_Cross/'
        voc_no = file.split('_')
...
def get_target(image, classes, confs, boxes, file):
    ...
    result_path = 'D:/KC_AOI_Project/wrk/study_wrk/predict_result/Predict_Pic_Cross_11_14/'
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    # 高斯處理
    pr_img_g = Image.open(root_path+str(voc_no[0])+ '.bmp')
    pr_img_g = np.array(pr_img_g)
    result_path_g = 'D:/KC_AOI_Project/wrk/study_wrk/predict_result/Predict_G_Pic_Cross_11_14/'
...
def doLocalOtsu(image,do_count,file):
    # 沒有高斯處理
    source = 'D:/KC_AOI_Project/wrk/study_wrk/predict_pic/KC_Pic_Cross_11_14/'
    # 检查文件夹是否存在
    ...
    # 有高斯處理
    gaus_source = 'D:/KC_AOI_Project/wrk/study_wrk/predict_pic/G_KC_Pic_Cross_11_14/'
    # 检查文件夹是否存在
