...
def excludeRectangle(image, x_start, y_start, x_end, y_end, excluded_value):
    height, width = image.shape[:2]
    for i in range(y_start, y_end):
        for j in range(x_start, x_end):
            if 0 <= i < height and 0 <= j < width:
                image[i, j] = excluded_value    
    return image
...
if __name__ == '__main__':
    ...
    # file_root = 'D:/KC_AOI_Project/wrk/study_wrk/G_KC_PIC/33_0.4.bmp'
    file_root = 'D:/KC_AOI_Project/wrk/study_wrk/G_KC_PIC/56_1.3.bmp'
    # file_root = 'D:/KC_AOI_Project/wrk/study_wrk/G_KC_PIC/35_0.8.bmp'
    # file_root = 'D:/KC_AOI_Project/wrk/study_wrk/G_KC_PIC/31_0.3.bmp'
    ...
    for (classid, conf, box) in zip(classes, confs, boxes):
        ...
        if (classid == 1):
            ...
            cut_img9898 = img[by:by + bheight, bx :bx + bwidth]
    print(cut_img9898[1,1],'測試看看')
    # 定义图像尺寸和每个矩形的边长（假设为 1/3）
    image_height, image_width = img.shape[:2]
    rect_length = min(image_height, image_width) // 3
    ...
    img = excludeRectangle(img, x_start, y_start, x_end, y_end, excluded_value)
    retval, new_image = cv2.threshold(img, 0, 255, cv2.THRESH_OTSU)
    # 使用OpenCV顯示圖像
    cv2.imshow('Excluded Image', new_image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    
    # 计算每个矩形的中心点坐标，并根据中心点遍历整个图像
    # threshold_values = []
    # for y in range(rect_length // 2, image_height, rect_length):
    #     for x in range(rect_length // 2, image_width, rect_length):
    #         print('x',x)
    #         print('y',y)
    #         # 检查当前像素是否位于类别1边界框内，如果是则跳过
    #         # if category1_bbox is not None:
    #         #     bx, by, bwidth, bheight = category1_bbox
    #         if bx <= x < bx + bwidth and by <= y < by + bheight:
    #             continue
    
    #         # 提取矩形范围内的像素值
    #         rect_pixels = img[y - rect_length // 2:y + rect_length // 2,
    #                                  x - rect_length // 2:x + rect_length // 2]
    
    #         # 使用Otsu算法计算阈值，并将阈值加入列表
    #         _, threshold_value = cv2.threshold(rect_pixels, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    #         threshold_values.append(threshold_value)
    #         # 将阈值赋值给img_copy对应矩形区域的像素值
    #         img_after[y - rect_length // 2:y + rect_length // 2, x - rect_length // 2:x + rect_length // 2] = threshold_value
    ...
    #         # 取得目標點所屬y軸點X向所有點
    #         x_row = img[i,x_start:x_end]
    #         # 取得目標點所屬x軸點上Y向所有點
    #         y_row = img[y_start:y_end,j]
    #         # 因目標點再鈐個row有取出 所以去除點重複的值
    #         y_row = np.delete(y_row, int(y_ridge))
    #         result1 = np.append(x_row, y_row)
    #         # local算R次方
    #         # print('olodoldold==>',image[i,j])
    #         # new_image[i,j] = image[i,j]
    #         new_image[i,j] = getLocalBestR(img[i,j], result1)
    #         # print('newnewnew==>',new_image[i,j])
    #         # local算otsu
    #         # result1 = cv2.GaussianBlur(result1, (3, 3), 0) 
    #         # retval, result1 = cv2.threshold(result1, 0, 255, cv2.THRESH_OTSU)
    ...
    # # # 打印计算得到的阈值
    # # print('Otsu threshold:', final_threshold)
