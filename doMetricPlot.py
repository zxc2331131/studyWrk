# -*- coding: utf-8 -*-
# grp_unetTrainCross.py  訓練unet model
# 資料集用 D:/tensorflow-unet-labelme-master/tensorflow-unet-labelme-master/datasets/trainCross
# 十字強化的
...
# 使用 Seaborn 库来提供更漂亮的图表样式
import seaborn as sns
sns.set(context='notebook', style='whitegrid', palette='pastel', font='sans-serif', font_scale=1.1, color_codes=True)
...
def plot_metrics(metrics1, metrics2, title, ylabel, label1="Dataset 1", label2="Dataset 2"):
    plt.figure(figsize=(10, 6))
    
    # 绘制第一组数据
    plt.plot(range(6, 15), metrics1, marker='o', linestyle='-', color='b', label=label1)  # 轻松的蓝色线条
    
    # 绘制第二组数据
    plt.plot(range(6, 15), metrics2, marker='s', linestyle='-', color='g', label=label2)  # 绿色线条
    
    plt.title(title, fontsize=14, fontweight='bold', color='navy')
    plt.xlabel("Dataset Version", fontsize=12, color='darkslategray')
    plt.ylabel(ylabel, fontsize=12, color='darkslategray')
    plt.xticks(range(6, 15), fontsize=10, color='darkred')  # assuming versions are integers
...
for i in range(6, 15):
    ...
    # global用
    pic_ver = '_'+f"{i:02}"
    dataset_path = 'D:/tensorflow-unet-labelme-master/tensorflow-unet-labelme-master/datasets/trainPlus'+pic_ver+'_voc'
    log_dir = 'modelLogs/Pluslogs'+pic_ver
    # log_dir = 'modelLogs/Pluslogs2'+pic_ver
    
    # LCGC用
    # pic_ver = '_11_'+f"{i:02}"
    
    # dataset_path = 'D:/tensorflow-unet-labelme-master/tensorflow-unet-labelme-master/datasets/trainCross'+pic_ver+'_voc'
    # log_dir = 'modelLogs/Crosslogs'+pic_ver
    # # log_dir = 'modelLogs/G_Pluslogs2'+pic_ver
    ...
    # Iterate over val_batches to get the true labels and predictions
    true_labels_list = []
    predicted_classes_list = []
    
    for input_batch, labels_batch in val_batches:
        # Make predictions
    ...
    # Flatten the list of arrays for true labels and predictions
    true_labels = np.vstack(true_labels_list).flatten()
    predicted_classes = np.vstack(predicted_classes_list).flatten()
    ...
    f_precisions.append(precision)
    f_accuracies.append(accuracy)
    f_f1_scores.append(f1)
    ...
    # 高斯
    dataset_path = 'D:/tensorflow-unet-labelme-master/tensorflow-unet-labelme-master/datasets/G_trainPlus'+pic_ver+'_voc'
    log_dir = 'modelLogs/G_Pluslogs'+pic_ver
    # log_dir = 'modelLogs/G_Pluslogs2'+pic_ver
    
    # dataset_path = 'D:/tensorflow-unet-labelme-master/tensorflow-unet-labelme-master/datasets/G_trainCross'+pic_ver+'_voc'
    # log_dir = 'modelLogs/G_Crosslogs'+pic_ver
    # # log_dir = 'modelLogs/G_Crosslogs2'+pic_ver
    ...
    print('{} model loaded.'.format(model_path))
    model.load_weights(model_path)
    current_learning_rate = model.optimizer.learning_rate
    ...
    # Print metrics
    print("G_Recall:", recall)
    print("G_Precision:", precision)
    print("G_Accuracy:", accuracy)
    print("G_F1 Score:", f1)
    
    # 计算指标并添加到列表
    g_recalls.append(recall)
    g_precisions.append(precision)
    g_accuracies.append(accuracy)
    g_f1_scores.append(f1)
#畫最終結果線性圖
plot_metrics(f_recalls, g_recalls, "Recall for global Dataset", "Recall", "gaus", "gerneral")
plot_metrics(f_precisions, g_precisions, "Precision for global Dataset", "Precision", "gaus", "gerneral")
plot_metrics(f_accuracies, g_accuracies, "Accuracy for global Dataset", "Accuracy", "gaus", "gerneral")
plot_metrics(f_f1_scores, g_f1_scores,"F1 Score for global Dataset", "F1 Score", "gaus", "gerneral")
